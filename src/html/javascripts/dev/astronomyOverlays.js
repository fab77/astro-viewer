...
export function wireAstronomyOverlayDemos() {
  const astronomy = document.querySelector(
    '[data-dev-tab-panel="astronomy"]'
  );
  ...
  panel.innerHTML = `
    ...
    <div class="stack" style="margin-top:8px;">
      <div class="hint">Local fixtures generated from ESASky TAP around M51.</div>
      <div class="row">
        <button id="btnLoadHscCatalogueDemo" class="secondary" type="button">Load HSC catalogue (~40)</button>
        <button id="btnLoadHstFootprintsDemo" class="secondary" type="button">Load HST observations (~40)</button>
      </div>
      <div class="hint">Hover a source or observation footprint to inspect its metadata below.</div>
    </div>`;

  const hoverPanel = el('hoverPanel');
  if (hoverPanel) astronomy.insertBefore(panel, hoverPanel);
  else astronomy.appendChild(panel);

  el('btnLoadHscCatalogueDemo')?.addEventListener('click', loadCatalogueFixture);
  el('btnLoadHstFootprintsDemo')?.addEventListener('click', loadFootprintFixture);
}
...
async function loadCatalogueFixture() {
  try {
    ...
    goToFixture(fixture);
    setStatus(`Loaded ${fixture.rows.length} HSC catalogue sources from ESASky fixture.`);
  } catch (error) {
    console.error(error);
...
async function loadFootprintFixture() {
  try {
    ...
    goToFixture(fixture);
    setStatus(`Loaded ${fixture.rows.length} HST observation footprints from ESASky fixture.`);
  } catch (error) {
    console.error(error);
...
async function loadFixture(url) {
  ...
  if (!response.ok) {
    throw new Error(`${response.status} ${response.statusText}. Generate fixtures with: node scripts/generate_esasky_demo_fixtures.mjs`);
  }
  const fixture = await response.json();
  if (!Array.isArray(fixture?.columns) || !Array.isArray(fixture?.rows) || fixture.rows.length === 0) {
...
function createMetadata(fixture, footprint = false) {
  ...
  const columns = fixture.columns.map((column, index) => {
    ...
    else if (column.name === fixture.mapping?.dec) columnType = ColumnType.GEOM_DEC;
    else if (footprint && column.name === fixture.mapping?.footprint) columnType = ColumnType.GEOM_FOOTPRINT;
    else if (column.name === fixture.mapping?.name) columnType = ColumnType.MAIN_NAME;
...
function goToFixture(fixture) {
  ...
  if (Number.isFinite(ra) && Number.isFinite(dec)) state.AstroAPI.goTo?.(ra, dec);
}
