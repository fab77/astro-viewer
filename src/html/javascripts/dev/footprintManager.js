...
export function renderFootprintManager() {
  ...
  const rows = state.FP_LIST
    ...
    .filter(({ f }) =>
      ...
      f.table?.toLowerCase().includes(filter)
    );

  if (!rows.length) {
    list.innerHTML = '<div class="hint catalogue-empty">No footprint sets loaded.</div>';
    return;
  }
    ...
    card.innerHTML = `
      <div class="catalogue-card-header">
        <div class="catalogue-card-heading">
          <div class="catalogue-card-title">${f.name || f.table || f.id || "(unnamed)"}</div>
          <div class="catalogue-card-key mono">${key}</div>
        </div>
      ...
      <div class="catalogue-card-footer">
        ...
        <div class="catalogue-card-actions">
          <button class="row-fp-del secondary"
            title="Remove from engine and STCS manager">Delete</button>
        </div>
      </div>`;
...
export function wireFootprintManagerControls() {
  ...
  el('fpTable')?.addEventListener('click', (ev) => {
    const card = ev.target.closest('.catalogue-card'); if (!card) return;
    ...
    if (ev.target.classList.contains('row-fp-del')) {
      try {
        ...
        persistBasic();
        setStatus(`🗑️ Footprint set deleted: ${fp.name || fp.id || fp.table}`);
