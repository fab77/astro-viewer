...
export class OBJMeshParser {
    ...
    for (const raw of lines) {
      ...
      if (line.startsWith('f ')) {
        ...
        for (let i = 1; i < face.length - 1; i++) {
          indices.push(
            first,
            OBJMeshParser.resolveIndex(face[i], vertices.length / 3),
            OBJMeshParser.resolveIndex(face[i + 1], vertices.length / 3),
          )
        }
      }
    }
  ...
  private static resolveIndex(objIndex: number, vertexCount: number): number {
