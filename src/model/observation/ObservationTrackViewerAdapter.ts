import { AstroViewer } from '../../AstroViewer.js'
import GeoJSONParser from '../../utils/GeoJSONParser.js'
import { ColumnType, MetadataColumn } from '../MetadataColumn.js'
import { MetadataManager } from '../MetadataManager.js'
import type { SensorConeGL } from '../terra/SensorConeGL.js'
import type { SatelliteObjectGL } from '../terra/SatelliteObjectGL.js'
import type { TerraFootprintSetGL } from '../terra/TerraFootprintSetGL.js'
import type { TerraPointSetGL } from '../terra/TerraPointSetGL.js'
import type { TerraPolylineSetGL } from '../terra/TerraPolylineSetGL.js'
import {
  createObservationTrackTimeline,
  footprintToRing,
} from './ObservationTrackTimeline.js'
import type {
...
type ViewerObjects = {
  target: TerraFootprintSetGL | null
  groundTrack: TerraPolylineSetGL | null
  footprints: TerraFootprintSetGL | null
  currentFootprint: TerraFootprintSetGL | null
  marker: TerraPointSetGL | null
  satelliteObject: SatelliteObjectGL | null
  sensorCone: SensorConeGL | null
}

const DEFAULT_VISUALISATION: Required<ObservationVisualisationLike> = {
...
const DEFAULT_COLORS: Required<ObservationTrackViewerColors> = {
  groundTrack: '#ffe066',
  footprints: '#00fff2',
...
export class ObservationTrackViewerAdapter {
  private activeHandle: InternalObservationTrackViewerHandle | null = null
  private readonly viewer: AstroViewer
  private readonly metadataManagerFactory: () => MetadataManager
  private readonly colors: Required<ObservationTrackViewerColors>
  ...
  load(track: ObservationTrackLike): ObservationTrackViewerHandle {
    this.clear()
    const handle = new InternalObservationTrackViewerHandle(
      this.viewer,
      track,
      this.metadataManagerFactory,
      this.colors,
      this.onFrame,
    )
...
class InternalObservationTrackViewerHandle implements ObservationTrackViewerHandle {
  readonly timeline: ObservationTrackTimelineController
  private readonly objects: ViewerObjects = {
    target: null,
    groundTrack: null,
    footprints: null,
  ...
  private visualisation: Required<ObservationVisualisationLike>
  private currentFootprintIndex = -1
  ...
  private createTargetOverlay(): TerraFootprintSetGL | null {
    const geojson = this.track.target?.geojson
    if (!geojson) return null

    const set = this.viewer.createTerraFootprintSet(
      this.track.target?.name ?? 'Observation target',
      'ObservationTrack target GeoJSON',
      this.track.target?.geojsonUrl ?? 'ObservationTrack',
      this.metadataManagerFactory(),
    )
  ...
  private createAllFootprintsOverlay(): TerraFootprintSetGL | null {
    const featureCollection = {
      type: 'FeatureCollection',
      features: this.track.samples.map((sample, index) => footprintFeature(sample, index)),
    }
    const set = this.viewer.createTerraFootprintSet(
      `${this.track.satellite?.name ?? 'Satellite'} footprints`,
      'ObservationTrack sampled sensor footprints',
      this.track.id ?? 'ObservationTrack',
      this.metadataManagerFactory(),
    )
  ...
  private createGroundTrackOverlay(): TerraPolylineSetGL | null {
    const set = this.viewer.createTerraPolylineSet(
      `${this.track.satellite?.name ?? 'Satellite'} ground track`,
      'ObservationTrack GroundTrackPoint[]',
      this.track.id ?? 'ObservationTrack',
      this.metadataManagerFactory(),
    )
    set.addGroundTrack(
      this.track.samples.map((sample) => ({
  ...
  private createMarkerOverlay(): TerraPointSetGL | null {
    const set = this.viewer.createTerraPointSet(
      `${this.track.satellite?.name ?? 'Satellite'} marker`,
      'ObservationTrack interpolated satellite marker',
      this.track.id ?? 'ObservationTrack',
      new MetadataManager(createMarkerColumns()),
    )
  ...
  private createSatelliteObject(): SatelliteObjectGL | null {
  ...
  private createSensorCone(): SensorConeGL | null {
  ...
  private updateFrame(frame: ObservationTrackFrame): void {
    if (this.disposed) return
  ...
  private updateMarker(point: GroundTrackPointLike): void {
  ...
  private updateSatelliteObject(frame: ObservationTrackFrame): void {
  ...
  private updateSensorCone(frame: ObservationTrackFrame): void {
    const sensorCone = this.objects.sensorCone
    if (!sensorCone) return
    sensorCone.setGeometry(frame.currentGroundPoint, frame.currentFootprint)
  }

  private updateCurrentFootprint(frame: ObservationTrackFrame): void {
    if (frame.nearestSampleIndex === this.currentFootprintIndex) return
  ...
  private createCurrentFootprintOverlay(sample: ObservationTimelineSample): TerraFootprintSetGL | null {
    const featureCollection = {
      type: 'FeatureCollection',
      features: [footprintFeature(sample, sample.index, 'Current footprint')],
    }
    const set = this.viewer.createTerraFootprintSet(
      `${this.track.satellite?.name ?? 'Satellite'} current footprint`,
      'ObservationTrack nearest sampled footprint',
      this.track.id ?? 'ObservationTrack',
      this.metadataManagerFactory(),
    )
    set.addGeoJSONFeatures(GeoJSONParser.parseGeoJSON(featureCollection))
  ...
  private applyVisualisation(): void {
    ...
    if (this.objects.target) this.viewer.hideTerraFootprintSet(this.objects.target, true)
    ...
    if (this.objects.marker) this.viewer.hideTerraPointSet(this.objects.marker, true)
  ...
  private deleteObjects(): void {
    if (this.objects.target) this.viewer.deleteTerraFootprintSet(this.objects.target)
    if (this.objects.groundTrack) this.viewer.deleteTerraPolylineSet(this.objects.groundTrack)
    if (this.objects.footprints) this.viewer.deleteTerraFootprintSet(this.objects.footprints)
    if (this.objects.currentFootprint) this.viewer.deleteTerraFootprintSet(this.objects.currentFootprint)
    if (this.objects.marker) this.viewer.deleteTerraPointSet(this.objects.marker)
    if (this.objects.satelliteObject) this.viewer.deleteSatelliteObject(this.objects.satelliteObject)
    if (this.objects.sensorCone) this.viewer.deleteSensorCone(this.objects.sensorCone)
...
function validateTrack(track: ObservationTrackLike): void {
  if (!track.samples || track.samples.length === 0) {
    throw new Error('ObservationTrackViewerAdapter requires at least one sample.')
  }
}

function footprintFeature(sample: ObservationSampleLike, index: number, name = `Footprint ${index + 1}`): Record<string, unknown> {
  return {
    type: 'Feature',
    properties: {
      name,
...
function footprintToGeoJSONGeometry(footprint: FootprintGeometryLike): Record<string, unknown> {
...
function ringToGeoJSON(
  ...
  return [...ring, [first[0], first[1]] as const]
}
