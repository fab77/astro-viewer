...
function mercatorYToLatDeg(yNormalized: number): number {
  const mercator = Math.PI * (1 - 2 * yNormalized)
  return (Math.atan(Math.sinh(mercator)) * 180) / Math.PI
}

function wrapLonToPhi(lonDeg: number): number {
  return lonDeg < 0 ? lonDeg + 360 : lonDeg
}

export class XYZMeshBuilder {
  buildTileMesh(tile: XYZTileCoord, segmentsPerSide = 16): XYZTileMesh {
    ...
    const indices =
      rawIndices.length > 65535 ? rawIndices : new Uint16Array(rawIndices)
